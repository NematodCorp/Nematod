...
void AddressSpace::write(address ptr, data value) {
    ...
    if(it == m_ports.begin()) {
        m_last_written_value = value; // Low address open bus special case
        return;
    }
    ...
    if(ptr < (it->base_address + it->module->size())) {
        it->module->write(ptr - it->base_address, value); // Relative address
    } else {
        m_last_written_value = value; // Open bus (modules aren't countiguous in memory)
    }
}
